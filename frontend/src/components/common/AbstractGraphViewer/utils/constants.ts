export const DEFAULT_NODE_RADIUS = 40;
export const DEFAULT_LINK_DISTANCE = 200;
export const DEFAULT_CHARGE_STRENGTH = -2000;
export const DEFAULT_COLLISION_RADIUS_MULTIPLIER = 20;
export const ZOOM_SCALE_EXTENT: [number, number] = [0.1, 4];
export const ZOOM_IN_FACTOR = 1.3;
export const ZOOM_OUT_FACTOR = 0.7;
export const FIT_TO_SCREEN_PADDING = 0.9;

// Forces de contrainte pour garder les nœuds dans une zone visible
export const BOUNDARY_FORCE_STRENGTH = 0.1;
export const CENTER_FORCE_STRENGTH = 0.05;
export const LINK_STRENGTH = 0.9;
export const ALPHA_TARGET = 0;
export const ALPHA_DECAY = 0.0228;
export const VELOCITY_DECAY = 0.4;

export const DEFAULT_EDGE_COLOR = "#999";
export const DEFAULT_NODE_COLOR = "#64748b";
export const SELECTED_NODE_COLOR = "#3b82f6";
export const SELECTED_NODE_STROKE_COLOR = "#1e40af";
export const DEFAULT_NODE_STROKE_COLOR = "#fff";
export const GRAPH_BACKGROUND_COLOR = "#f9fafb"; // Couleur de fond du graphe (gris très clair)

export const SELECTED_STROKE_WIDTH = 3;
export const DEFAULT_STROKE_WIDTH = 2;
export const EDGE_STROKE_WIDTH = 2;
export const EDGE_STROKE_OPACITY = 0.6;
export const EDGE_ARROW_OFFSET = 8; // Espace entre la fin du path et le nœud cible

export const NODE_LABEL_FONT_SIZE = 12;
export const EDGE_LABEL_FONT_SIZE = 10;
export const NODE_LABEL_OFFSET = 15;
export const EDGE_LABEL_OFFSET = -5;
export const NODE_LABEL_MAX_LENGTH = 10; // Nombre maximum de caractères avant troncature
