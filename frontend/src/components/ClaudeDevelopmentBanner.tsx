...
interface ClaudeDevelopmentBannerProps {
  tickets: Ticket[];
  developing: boolean;
  onDevelopWithClaude: (ticketId: string) => void;
}

export function ClaudeDevelopmentBanner({ tickets, developing, onDevelopWithClaude }: ClaudeDevelopmentBannerProps) {
  ...
  return (
    <div className="relative overflow-hidden bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="p-5 space-y-3">
        ...
        <Button
          onClick={() => onDevelopWithClaude(nextTicket.id)}
          disabled={developing}
          size="lg"
          className="w-full bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white font-semibold shadow-lg hover:shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {developing ? (
            <>
