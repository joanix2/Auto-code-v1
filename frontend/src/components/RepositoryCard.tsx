  ...
  return (
    <Card className="hover:shadow-lg transition-shadow duration-200 border-slate-200 dark:border-slate-800 flex flex-col">
      ...
      <CardContent className="flex flex-col flex-1 space-y-3">
        ...
        {/* Date */}
        <div className="text-xs text-slate-500 border-t border-slate-200 dark:border-slate-800 pt-2 space-y-1">
          {repository.github_created_at && (
            <div className="flex items-center gap-2">
              <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
              </svg>
              <span>Créé le {formatDate(repository.github_created_at)}</span>
            </div>
          )}
          {repository.github_pushed_at && (
            <div className="flex items-center gap-2">
              ...
              <span>Dernier commit le {formatDate(repository.github_pushed_at)}</span>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col gap-2">
          {repository.url && (
            <a href={repository.url} target="_blank" rel="noopener noreferrer" className="w-full">
          ...
          <Link to={`/repository/${repository.id}/tickets`} className="w-full">
