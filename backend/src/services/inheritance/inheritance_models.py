...
class InheritanceTree(BaseModel):
    ...
    parent_id: str | None = Field(default=None, description="ID of the parent (None if root)")
    depth: int = Field(default=0, ge=0, description="Depth from the root of the chain")
...
class InheritedElement(BaseModel):
    ...
    element_type: str = Field(..., description="Type of element: 'node' or 'edge'")
    source_graph_id: str = Field(..., description="ID of the originating graph")
