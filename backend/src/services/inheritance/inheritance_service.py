...
class InheritanceService:
    ...
    def resolve_inheritance(
        ...
        if "allowed_node_types" in parent_meta:
            existing = {nt["name"] for nt in meta.get("allowed_node_types", [])}
        ...
        if "allowed_edge_types" in parent_meta:
            existing = {et["name"] for et in meta.get("allowed_edge_types", [])}
    ...
    def get_inheritance_chain(
        ...
        while depth <= max_depth:
            ...
            if graph_id in visited:
                raise ValueError(f"Circular inheritance detected at graph '{graph_id}'")
            ...
            inh_type = InheritanceType(inh_type_str) if inh_type_str else None
    ...
    def get_element_origin(
        ...
        is_overridden = (in_child_nodes and in_parent_nodes) or (in_child_edges and in_parent_edges)
    ...
    def resolve_multiple_inheritance(
        ...
        existing_node_types = {nt["name"] for nt in meta.get("allowed_node_types", [])}
        for parent in parent_graphs:
        ...
        existing_edge_types = {et["name"] for et in meta.get("allowed_edge_types", [])}
        for parent in parent_graphs:
            pmeta = parent.get(META_KEY, {})
