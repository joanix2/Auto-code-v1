...
class MessageService(GitHubSyncService[Message]):
    ...
    async def _update_in_db(self, entity_id: str, data: dict[str, Any]) -> Message | None:
    ...
    async def get_all(self, filters: dict[str, Any] | None = None) -> list[Message]:
    ...
    async def delete_on_github(
        ...
        **kwargs,
        ...
        if not entity or not entity.github_comment_id:
            logger.warning("Cannot delete message on GitHub: no comment ID")
            return True  # Allow DB deletion anyway

        repository_full_name = kwargs.get("repository_full_name")
        if not repository_full_name:
            logger.warning("Cannot delete message on GitHub: no repository_full_name")
            return True
    ...
    async def sync_from_github(
        ...
        **kwargs,
        ...
        if not number:
            raise ValueError(
                "pr_number or issue_number is required (or provide issue_id with issue_repo)"
            )
        ...
        github_comments = await self.fetch_from_github_api(
            access_token, owner, repo_name, pr_number=number
        )
    ...
    async def create_message(
        self, issue_id: str, content: str, author_username: str, author_type: str = "user"
    ) -> Message:
