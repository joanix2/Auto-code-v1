...
class RepositoryService(GitHubSyncService[Repository]):
    ...
    async def _update_in_db(self, entity_id: str, data: dict[str, Any]) -> Repository | None:
    ...
    async def get_by_id(self, entity_id: str) -> Repository | None:
        """Get repository by ID"""
        return await self.repo_repository.get_by_id(entity_id)

    async def get_all(self, filters: dict[str, Any] | None = None) -> list[Repository]:
    ...
    async def fetch_from_github_api(
        self, access_token: str, username: str | None = None, **kwargs
    ) -> list[dict[str, Any]]:
        """
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(
                url, headers=headers, params={"per_page": 100, "sort": "updated"}
            )
