...
class PlanExecutor:
    ...
    def execute_plan(
        ...
        if plan.execution_mode == ExecutionMode.PARALLEL:
            step_results = self._execute_parallel(sorted_steps, plan, qs, output_files)
            ...
            for step in sorted_steps:
                ...
                if not result.success:
                    if plan.error_strategy == ErrorStrategy.ABORT:
                        abort = True
                    elif plan.error_strategy == ErrorStrategy.RETRY and result.error:
    ...
    def _render_step(
        ...
        if step.entity_id:
            ...
            if entity_tree is None:
                raise ValueError(f"Entity '{step.entity_id}' not found for step '{step.name}'")
    ...
    def _compute_levels(
        ...
        while remaining:
            ...
            if not current_level:
                ...
                raise ValueError(f"Cannot compute levels for remaining steps: {remaining}")
