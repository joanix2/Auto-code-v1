...
class InferenceEngine:
    ...
    def run_fixpoint(self, ontology: OntologyGraph, max_iterations: int = 10) -> list[Fact]:
        ...
        for iteration in range(1, max_iterations + 1):
            ...
            if not new_facts:
                logger.info(f"Fixpoint reached at iteration {iteration} (no new facts generated)")
                break
    ...
    def _apply_rule(self, rule: InferenceRule, ontology: OntologyGraph) -> list[Fact]:
        ...
        if not condition_patterns:
            logger.warning(f"Rule '{rule.id}' has unparseable condition: {rule.condition}")
            return []
        ...
        for match in matches:
            ...
            source_confidences = [
                ontology.get_facts_by_statement(s) for s in match.get("_source_statements", [])
            ]
            base_confidence = self._compute_base_confidence(source_confidences)
            confidence = base_confidence * rule.confidence_discount

            # Build justification
            justification = self._build_justification(rule, match, base_confidence, confidence)
    ...
    def _find_matches(
        ...
        for fs in fact_statements:
            binding = InferenceEngine._match_statement(fs["statement"], first_pred, first_args)
        ...
        for pattern in patterns[1:]:
            ...
            for existing_match in matches:
                ...
                for fs in fact_statements:
                    ...
                    concrete_args = [existing_match.get(a, a) for a in args]
                    binding = InferenceEngine._match_statement(fs["statement"], pred, concrete_args)
                    if binding is not None:
                        ...
                        for k, v in binding.items():
                            ...
                            merged[k] = v
                        if not conflict:
                            merged["_source_statements"] = existing_match.get(
                                "_source_statements", []
                            ) + [fs["statement"]]
    ...
    @staticmethod
    def _match_statement(statement: str, predicate: str, args: list[str]) -> dict[str, str] | None:
    ...
    @staticmethod
    def _instantiate_template(template: str, bindings: dict[str, str]) -> str:
