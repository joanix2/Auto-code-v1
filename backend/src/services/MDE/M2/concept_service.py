...
class ConceptService(BaseService[Concept]):
    ...
    async def update(self, concept_id: str, update_data: dict[str, Any]) -> Concept | None:
        """
        Update a concept
        ...
        """
        logger.info(
            f"🔍 ConceptService.update called with concept_id={concept_id}, update_data={update_data}"
        )
        ...
        if "name" in updates:
            ...
            if existing and existing.id != concept_id:
                raise ValueError(
                    f"Concept with name '{updates['name']}' already exists in this metamodel"
                )
