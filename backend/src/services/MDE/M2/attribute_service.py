...
class AttributeService(BaseService[Attribute]):
    ...
    async def update(self, attribute_id: str, update_data: dict[str, Any]) -> Attribute | None:
        """
        Update an attribute
        ...
        if "name" in updates:
            ...
            if attribute:
                existing = await self.attribute_repo.get_by_name(
                    attribute.concept_id, updates["name"]
                )
                if existing and existing.id != attribute_id:
                    raise ValueError(
                        f"Attribute with name '{updates['name']}' already exists in this concept"
                    )
