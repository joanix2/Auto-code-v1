...
class RelationshipService(BaseService[Relationship]):
    ...
    async def _create_inverse_relationship(
        self, relationship: Relationship, source_id: str, target_id: str
    ) -> Relationship | None:
        """
    ...
    async def get_all(self, filters: dict[str, Any] | None = None) -> list[Relationship]:
        """
        ...
        if "metamodel_id" in filters:
            return await self.get_by_metamodel(
                filters["metamodel_id"], skip, limit, include_inverse
            )
