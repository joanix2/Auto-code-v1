...
from src.models.dsl import DSLEdgeCreate, DSLEdgeType, DSLEdgeUpdate
...
# Router definition
router = APIRouter(prefix="/api/dsls/edges", tags=["edges"])


def get_edge_repository(db=Depends(get_db)):
    """Dependency to get edge repository"""
    return DSLEdgeRepository(db)


@router.post("/", status_code=status.HTTP_201_CREATED)
async def create_edge(
    edge_data: DSLEdgeCreate,
    current_user: User = Depends(get_current_user),
    db=Depends(get_db),
    edge_repo: DSLEdgeRepository = Depends(get_edge_repository),
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if isinstance(edge_data.edge_type, str):
            edge_type_enum = DSLEdgeType(edge_data.edge_type.lower())
        else:
            edge_type_enum = edge_data.edge_type
    except ValueError:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Invalid edge type: {edge_data.edge_type}. Must be one of: {[e.value for e in DSLEdgeType]}",
        )
...
async def update_edge(
    ...
    try:
        edge_type_enum = DSLEdgeType(edge_type_str.lower())
    except ValueError:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Invalid edge type: {edge_type_str}. Must be one of: {[e.value for e in DSLEdgeType]}",
        )
...
async def delete_edge(
    ...
    try:
        ...
        edge_type_enum = DSLEdgeType(edge_type.lower())
    except ValueError:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Invalid edge type: {edge_type}. Must be one of: {[e.value for e in DSLEdgeType]}",
        )
