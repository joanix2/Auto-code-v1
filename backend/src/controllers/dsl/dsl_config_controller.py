...
from src.models.dsl.dsl_config import DSLConfig

router = APIRouter(prefix="/api/dsl-config", tags=["M3"])
...
async def get_node_types() -> list[NodeType]:
    ...
    """
    return DSLConfig.get_node_types()
...
async def get_node_type(type_id: str) -> NodeType:
    ...
    Returns:
        NodeType object
    """
    node_type = DSLConfig.get_node_type(type_id)
    if not node_type:
...
async def get_edge_types() -> list[EdgeType]:
    ...
    """
    return DSLConfig.get_edge_types()
...
async def get_edge_type(type_id: str) -> EdgeType:
    ...
    Returns:
        EdgeType object
    """
    edge_type = DSLConfig.get_edge_type(type_id)
    if not edge_type:
        from fastapi import HTTPException
...
async def get_edge_constraints() -> list[dict[str, Any]]:
    ...
    """
    return DSLConfig.get_edge_constraints()


@router.get("/config")
async def get_dsl_config() -> dict[str, Any]:
    """
    ...
    """
    return {
        "nodeTypes": DSLConfig.get_node_types(),
        "edgeTypes": DSLConfig.get_edge_types(),
        "edgeConstraints": DSLConfig.get_edge_constraints(),
    }
