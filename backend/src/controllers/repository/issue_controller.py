...
class IssueController(BaseController[Issue, IssueCreate, IssueUpdate]):
    ...
    async def validate_update(
        self, resource_id: str, updates: IssueUpdate, current_user: User, db
    ) -> dict[str, Any]:
    ...
    async def validate_delete(self, resource_id: str, current_user: User, db) -> dict[str, Any]:
        ...
        if not repository or not repository.full_name:
            logger.warning(
                f"Cannot sync issue {resource_id} deletion to GitHub: repository full_name missing"
            )
    ...
    async def delete(self, resource_id: str, current_user: User, db) -> dict[str, str]:
        ...
        try:
            ...
            logger.info(
                f"🚀 Calling service.delete with: resource_id={resource_id}, has_token={bool(access_token)}, kwargs={validated_data}"
            )
...
async def assign_to_copilot(
    ...
    try:
        if not current_user.github_token:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="GitHub not connected"
            )
        ...
        if not issue.github_issue_number:
            ...
            await issue_repository.link_to_github(
                issue_id,
                {"github_issue_number": gh_issue.number, "github_issue_url": gh_issue.html_url},
            )
