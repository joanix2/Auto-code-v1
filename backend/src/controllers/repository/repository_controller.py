...
class RepositoryController(
    ...
    async def create(self, data: RepositoryCreate, current_user: User, db) -> Repository:
        ...
        try:
            ...
            error_detail = (
                e.response.json().get("message", "Unknown error") if e.response else str(e)
            )
            raise HTTPException(
    ...
    async def update(
        self, resource_id: str, updates: RepositoryUpdate, current_user: User, db
    ) -> Repository:
    ...
    async def validate_create(
        self, data: RepositoryCreate, current_user: User, db
    ) -> dict[str, Any]:
    ...
    async def delete(self, resource_id: str, current_user: User, db) -> dict[str, str]:
        ...
        try:
            ...
            deleted = await self.service.delete(
                entity_id=resource_id, access_token=current_user.github_token
            )
...
async def get_repository(
    ...
    if repository.owner_username != current_user.username:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized to access this repository"
        )
...
async def sync_repository_issues(
    repository_id: str, current_user: User = Depends(get_current_user), db=Depends(get_db)
):
    ...
    if repository.owner_username != current_user.username:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized to sync this repository"
        )
