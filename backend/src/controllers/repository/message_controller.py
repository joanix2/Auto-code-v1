...
class MessageController(BaseController[Message, MessageCreate, None]):
    ...
    async def validate_create(self, data: MessageCreate, current_user: User, db) -> dict[str, Any]:
        ...
        if not repository.full_name:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Repository full_name is missing"
            )
    ...
    async def validate_update(
        self, resource_id: str, updates, current_user: User, db
    ) -> dict[str, Any] | None:
        """Messages cannot be updated for now"""
        raise HTTPException(
            status_code=status.HTTP_405_METHOD_NOT_ALLOWED, detail="Messages cannot be updated"
        )
    ...
    async def validate_delete(self, resource_id: str, current_user: User, db) -> dict[str, Any]:
        ...
        if not issue or not issue.repository_id:
            logger.warning(
                f"Cannot sync message {resource_id} deletion to GitHub: issue or repository not found"
            )
            # Return minimal data, will skip GitHub sync
            return {"access_token": current_user.github_token, "entity_id": resource_id}

        repository = await self.repository_repository.get_by_id(issue.repository_id)
        if not repository or not repository.full_name:
            logger.warning(
                f"Cannot sync message {resource_id} deletion to GitHub: repository full_name missing"
            )
            return {"access_token": current_user.github_token, "entity_id": resource_id}
    ...
    async def create(self, data: MessageCreate, current_user: User, db) -> Message:
        ...
        try:
            ...
            raise HTTPException(
                status_code=e.response.status_code, detail=f"GitHub API error: {e.response.text}"
            )
    ...
    async def delete(self, resource_id: str, current_user: User, db) -> dict[str, str]:
        ...
        try:
            ...
            await self.service.delete(
                resource_id, validated_data.get("access_token"), **validated_data
            )
