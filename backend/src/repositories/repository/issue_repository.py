...
class IssueRepository(BaseRepository[Issue]):
    ...
    async def get_by_github_issue_number(
        ...
        result = self.db.execute_query(
            query, {"repository_id": repository_id, "issue_number": issue_number}
        )
        if not result:
            return None
        return self.model(**convert_neo4j_types(result[0]["n"]))

    async def link_to_github(self, issue_id: str, github_data: dict) -> Issue | None:
        """
    ...
    async def assign_to_copilot(self, issue_id: str, assigned: bool = True) -> Issue | None:
        """
        ...
        if assigned:
            ...
            return await self.update(
                issue_id, {"assigned_to_copilot": False, "copilot_started_at": None}
            )

    async def get_copilot_issues(self, repository_id: str | None = None) -> list[Issue]:
        """
